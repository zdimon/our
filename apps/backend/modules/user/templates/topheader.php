  ...
  if ($sf_user->isAuthenticated())
{
  ...
  //    $data.= '<li style="float: left;"><a title="'.$i->getsfGuardUser()->getProfile()->getImgSeo().'" href="'.url_for('profile/show?username='.$i->getsfGuardUser()->getUsername()).'"><span class="joinMeBule">'.$arr[rand(0,2)].'</span>'.image_tag($i->getUrlImageMiddle(),array('alt'=>$i)).'</a></li>';
  // }
  $data .= '</ul>';
