...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    $sqls = array(
	 ...
	 $sqls = array(
      "CREATE TABLE catalogue (cat_id SERIAL, name VARCHAR(100), source_lang VARCHAR(100), target_lang VARCHAR(100), date_created BIGINT, date_modified BIGINT, author VARCHAR(255), PRIMARY KEY(cat_id))",
      "CREATE TABLE trans_unit (msg_id SERIAL, cat_id BIGINT DEFAULT '1', source VARCHAR(2500), target  VARCHAR(2500), comments  VARCHAR(2500), date_added BIGINT DEFAULT 0, date_modified BIGINT DEFAULT 0, author VARCHAR(255), translated INT, PRIMARY KEY(msg_id))",
      "ALTER TABLE trans_unit ADD CONSTRAINT ref_cat_id FOREIGN KEY (cat_id) REFERENCES catalogue(cat_id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE;",
    );
